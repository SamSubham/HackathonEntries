...
train <- read.csv("C:/Users/Sam/Desktop/AEGIS Classes/ML_whole/DataDriven/train.csv",header = T)

# Importing train labels data
train_labels <- read.csv("C:/Users/Sam/Desktop/AEGIS Classes/ML_whole/DataDriven/trainlabels.csv",header = T)

# Importing test data
test <- read.csv("C:/Users/Sam/Desktop/AEGIS Classes/ML_whole/DataDriven/test.csv",header = T)

#######################################################MERGING THE DATASETS###################################################################

# Mergeing the data frames to create the dataframe train
train <- merge(train_labels, train)
test <- test
rm(train_labels)  
rm(train)
rm(test)

# checking the no. of pumps in each functional status group
table(train$status_group)

# checking the no. of pumps in each functional status group as proportions
...
qplot(status_group, data=train, geom="bar", fill=quantity) + theme(legend.position = "top")
...
ggplot(train, aes(x = construction_year)) + geom_histogram(bins = 20) + facet_grid( ~ status_group) 

# subsetting when construction_year is larger than 0
...
#removeing these variables as first model run shows they are not important:
#  +tsh0flag, +gps0flag, +pop01flag, +public_meeting, +permit, +year0flag, +management_group, +quality_group
                                 

#second model run has these variables as least important
#source_type, basin, scheme_management
...
qplot(quantity_group, data=trainm, geom="bar", fill=status_group) + theme(legend.position = "top")

qplot(status_group, data=trainm, geom="bar", fill=quantity_group) + theme(legend.position = "top")

qplot(waterpoint_type_group, data=trainm, geom="bar", fill=status_group) + theme(legend.position = "top")

qplot(extraction_type_class, data=trainm, geom="bar", fill=status_group) + theme(legend.position = "top") + theme(axis.text.x=element_text(angle = -20, hjust = 0))

# Create a histograms for continuous variables
ggplot(trainm, aes(x = finallongitude)) + geom_histogram(bins = 20) +   facet_grid( ~ status_group)

ggplot(trainm, aes(x = latitude)) + geom_histogram(bins = 20) +   facet_grid( ~ status_group)

ggplot(trainm, aes(x = latitude)) + geom_histogram(bins = 20) +   facet_grid( ~ status_group)

ggplot(trainm, aes(x = gps_height)) + geom_histogram(bins = 20) +   facet_grid( ~ status_group)

ggplot(trainm, aes(x = age)) + geom_histogram(bins = 20) +   facet_grid( ~ status_group)

ggplot(trainm, aes(x = population)) + geom_histogram(bins = 20) +   facet_grid( ~ status_group)

ggplot(trainm, aes(x = amount_tsh)) + geom_histogram(bins = 10) +   facet_grid( ~ status_group)
...
write.csv(submissionForest,file="Submission_R.csv",row.names=FALSE)
